...
def create_gradio_interface():
    ...
    with gr.Blocks(title="Qwen3 Text Generation") as gradio_app:
        ...
        with gr.Row():
            with gr.Column():
                input_text = gr.Textbox(label="Input Message", placeholder="Enter your message here...", lines=5)
                generate_btn = gr.Button("Generate", variant="primary")

            with gr.Column():
                thinking_output = gr.Textbox(label="Thinking Process", lines=10, interactive=False)
                content_output = gr.Textbox(label="Generated Content", lines=10, interactive=False)

        with gr.Row():
            status_btn = gr.Button("Check Status")
            status_output = gr.Textbox(label="Service Status", lines=2, interactive=False)

        # Connect buttons to functions
        generate_btn.click(fn=gradio_generate, inputs=[input_text], outputs=[thinking_output, content_output])

        status_btn.click(fn=gradio_check_status, inputs=[], outputs=[status_output])
